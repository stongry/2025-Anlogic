source "D:/Anlogic/TD_6.2.1_Engineer_6.2.168.116/cw/atpl/templa.tcl"
set fd [open "D:/Anlogic/TD_6.2.1_Engineer_6.2.168.116/cw/atpl/cwc.atpl" r]
set tmpl [read $fd]
close $fd
set parser [::tmpl_parser::tmpl_parser $tmpl]

set ComponentName        auto_created_cwc2
set bus_num              12
set cwc_ctrl_len         212
set cwc_bus_ctrl_len     192
set bus_din_num          54
set ram_len              54
set input_pipe_num       0
set output_pipe_num      0
set depth                1024
set capture_ctrl_exist   0
set bus_width            { 1,1,1,8,8,8,1,8,1,8,8,1 };
set bus_din_pos          { 0,1,2,3,11,19,27,28,36,37,45,53 };
set bus_ctrl_pos         { 0,4,8,12,40,68,96,100,128,132,160,188 };
set fp [open "cw/auto_created_cwc2/auto_created_cwc2_watcherInst.sv" w+]
puts $fp [eval $parser]
close $fp
