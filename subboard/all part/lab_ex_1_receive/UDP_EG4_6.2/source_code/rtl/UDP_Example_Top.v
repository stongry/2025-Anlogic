...
parameter  LOCAL_MAC_ADDRESS  = 48'h0123456789ba;
...
// 数字解析和显示相关信号
wire signed [8:0] parsed_digit_value;  // 修改为9位有符号数：-100到+100
wire parsed_digit_valid;
wire [3:0] parsed_led_control_value;
wire parsed_led_control_valid;

// UDP数据包解析器
udp_display_parser u_udp_display_parser(
    .clk                (udp_clk                ),
    .reset              (reset                  ),
    .app_rx_data_valid  (app_rx_data_valid      ),
    .app_rx_data        (app_rx_data            ),
    .app_rx_data_length (app_rx_data_length     ),
    .digit_value        (parsed_digit_value     ),
    .digit_value_valid  (parsed_digit_valid     ),
    .led_control_value  (parsed_led_control_value),
    .led_control_valid  (parsed_led_control_valid)
);

// 数值分解：将-100~100分解为符号位、百位、十位、个位
wire is_negative;
wire [8:0] abs_value;  // 改为9位，匹配parsed_digit_value的位宽
wire [3:0] digit_hundreds;
wire [3:0] digit_tens;
wire [3:0] digit_ones;

// 使用标准的有符号数判断和绝对值计算
assign is_negative = (parsed_digit_value < 9'sd0);  // 明确指定有符号比较
// 绝对值计算：使用9位来避免截断
assign abs_value = is_negative ? (-parsed_digit_value) : parsed_digit_value;
assign digit_hundreds = (abs_value >= 9'd100) ? 4'd1 : 4'd0;  // 百位（100时为1，其他为0）
assign digit_tens = ((abs_value % 9'd100) / 9'd10);           // 十位
assign digit_ones = (abs_value % 9'd10);                      // 个位

// 7段数码管解码器 - 个位
wire [6:0] seg_code_ones;
seg_decoder u_seg_decoder_ones(
    .bin_data   (digit_ones              ),
    .seg_data   (seg_code_ones           )
);

// 7段数码管解码器 - 十位
wire [6:0] seg_code_tens;
seg_decoder u_seg_decoder_tens(
    .bin_data   (digit_tens              ),
    .seg_data   (seg_code_tens           )
);

// 7段数码管解码器 - 百位
wire [6:0] seg_code_hundreds;
seg_decoder u_seg_decoder_hundreds(
    .bin_data   (digit_hundreds          ),
    .seg_data   (seg_code_hundreds       )
);

// 7段数码管解码器 - 符号位
wire [6:0] seg_code_sign;
seg_decoder u_seg_decoder_sign(
    .bin_data   (is_negative ? 4'hF : 4'hA),  // 负数显示"-"(0xF)，正数显示空白(0xA)
    .seg_data   (seg_code_sign               )
);

// 7段数码管扫描控制器（多位显示）
// 显示格式（从右到左）：
//   seg_data_0: 百位（abs_value == 100时显示）- 硬件最左边
//   seg_data_1: 符号位（负数时显示"-"）
//   seg_data_2: 十位（abs_value >= 10时显示）
//   seg_data_3: 个位（始终显示）- 硬件最右边
seg_scan u_seg_scan(
    .clk        (clk_25_out                  ),
    .rst_n      (key1                        ),
    .seg_data_0 ((abs_value >= 9'd100) ? {1'b1, seg_code_hundreds} : 8'b11111111), // 百位
    .seg_data_1 (is_negative ? {1'b1, seg_code_sign} : 8'b11111111), // 符号位
    .seg_data_2 ((abs_value >= 9'd10) ? {1'b1, seg_code_tens} : 8'b11111111),  // 十位
    .seg_data_3 ({1'b1, seg_code_ones}       ), // 个位：始终显示
    .seg_data_4 (8'b11111111                 ), // 关闭
    .seg_data_5 (8'b11111111                 ), // 关闭
    .seg_data   (seg_data                    ),
    .seg_sel    (seg_sel                     )
);

// LED控制逻辑
reg [3:0] led_control_reg;
always @(posedge udp_clk or posedge reset) begin
    if(reset) begin
        led_control_reg <= 4'b0000; // 初始状态：LED全灭
    end
    else if(parsed_led_control_valid) begin
        led_control_reg <= parsed_led_control_value;
    end
end

assign led_data = led_control_reg;
...
(   
    ...
    .input_local_ip_address     (input_local_ip_address),//input_local_ip_address     ),
    .input_local_ip_address_valid(input_local_ip_address_valid),//input_local_ip_address_valid),
