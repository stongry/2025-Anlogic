...
// 批次发送控制寄存器
reg [18:0] batch_start_addr;  // 当前批次起始地址
reg batch_sending;             // 批次发送标志

// 定义批次发送参数
localparam BATCH_SIZE = 19'd2000;  // 每批发送2000个像素（6000字节）
localparam FRAME_SIZE = 19'd307200; // 总帧大小
	...
	always @(posedge clk or negedge rst_n)
		begin
			...
			rd_done <=1'b0;
            app_rd_en_reg<=1'b0;
            batch_start_addr <= 19'b0;
            batch_sending <= 1'b0;
			end
			// 检查是否需要开始新的批次发送
			else if(!batch_sending & !Sdr_busy & (sdr_rd_delay== 1000) & !rd_done & (udp_wrusedw < 'd100) & Sdr_init_ref_vld == 1'b0)
			begin
				batch_sending <= 1'b1;
				r_addr_cnt <= batch_start_addr;
			end
			// 批次发送中
			else if(batch_sending & !Sdr_busy & (r_addr_cnt < batch_start_addr + BATCH_SIZE) & (r_addr_cnt < FRAME_SIZE) & Sdr_init_ref_vld == 1'b0 & burst_sdr_rd_cnt == 2'b0)
			begin
            r_addr_cnt <=r_addr_cnt +1'b1;
			app_rd_en_reg <= 1'b1;
            end
			else if (batch_sending & !Sdr_busy & (r_addr_cnt < batch_start_addr + BATCH_SIZE) & (r_addr_cnt < FRAME_SIZE) & Sdr_init_ref_vld == 1'b0 & (burst_sdr_rd_cnt > 0 &  burst_sdr_rd_cnt < 3))
			begin
            r_addr_cnt <=r_addr_cnt +1'b1;
			app_rd_en_reg <= 1'b1;
            end
			else if (batch_sending & !Sdr_busy & Sdr_init_ref_vld == 1'b0 & burst_sdr_rd_cnt == 3)
			begin
			app_rd_en_reg <= 1'b0;
            end
            // 批次发送完成
            else if(batch_sending & (r_addr_cnt >= batch_start_addr + BATCH_SIZE | r_addr_cnt >= FRAME_SIZE))
			begin
				batch_sending <= 1'b0;
				app_rd_en_reg <= 1'b0;
				// 更新下一批次的起始地址
				if(r_addr_cnt >= FRAME_SIZE) begin
					// 一帧发送完成，重新开始
					batch_start_addr <= 19'b0;
					rd_done <= 1'b1;  // 标记帧完成
				end
