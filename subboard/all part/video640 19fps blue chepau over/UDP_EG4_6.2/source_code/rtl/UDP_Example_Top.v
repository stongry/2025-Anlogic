...
// 使用对齐后的信号
wire [7:0] image_r_raw = Sdr_rd_dout_aligned[23:16];
wire [7:0] image_g_raw = Sdr_rd_dout_aligned[15:8];
wire [7:0] image_b_raw = Sdr_rd_dout_aligned[7:0];
wire [7:0] image_r_filtered;
wire [7:0] image_g_filtered;
wire [7:0] image_b_filtered;
wire       image_is_klein_blue;
wire [7:0] image_r_overlay;
wire [7:0] image_g_overlay;
wire [7:0] image_b_overlay;
wire        bbox_frame_start;
wire        bbox_frame_end;
wire        bbox_has;
wire [9:0]  bbox_min_x_prev_w;
wire [9:0]  bbox_max_x_prev_w;
wire [8:0]  bbox_min_y_prev_w;
wire [8:0]  bbox_max_y_prev_w;
wire [9:0]  divider_x0_w;
wire [9:0]  divider_x1_w;
wire [9:0]  divider_x2_w;
wire [9:0]  divider_x3_w;
wire [9:0]  divider_x4_w;
wire [9:0]  divider_x5_w;
// wire [3:0]  digit_value0;
// wire [3:0]  digit_value1;
// wire [3:0]  digit_value2;
// wire [3:0]  digit_value3;
// wire [3:0]  digit_value4;
// wire        digits_valid;
reg  [2:0]  frame_gap_cnt;
wire        Sdr_rd_en_for_udp;

klein_blue_replace u_klein_blue_replace (
    .in_r (image_r_raw),
    .in_g (image_g_raw),
    .in_b (image_b_raw),
    .out_r(image_r_filtered),
    .out_g(image_g_filtered),
    .out_b(image_b_filtered),
    .is_target_blue(image_is_klein_blue)
);
...
blue_bbox_overlay #(
    ...
    .in_r       (image_r_filtered),
    .in_g       (image_g_filtered),
    .in_b       (image_b_filtered),
    .is_blue    (image_is_klein_blue),
    .out_r      (image_r_overlay),
    .out_g      (image_g_overlay),
    .out_b      (image_b_overlay),
    .frame_start_o (bbox_frame_start),
    .frame_end_o   (bbox_frame_end),
    .has_bbox      (bbox_has),
    .bbox_min_x    (bbox_min_x_prev_w),
    .bbox_max_x    (bbox_max_x_prev_w),
    .bbox_min_y    (bbox_min_y_prev_w),
    .bbox_max_y    (bbox_max_y_prev_w),
    .divider_x0_o  (divider_x0_w),
    .divider_x1_o  (divider_x1_w),
    .divider_x2_o  (divider_x2_w),
    .divider_x3_o  (divider_x3_w),
    .divider_x4_o  (divider_x4_w),
    .divider_x5_o  (divider_x5_w)
);

always @(posedge sdr_clk or posedge reset) begin
    if (reset) begin
        frame_gap_cnt <= 3'd0;
    end else begin
        if (bbox_frame_end)
            frame_gap_cnt <= 3'd2;
        else if ((frame_gap_cnt != 3'd0) && ~Sdr_rd_en_aligned)
            frame_gap_cnt <= frame_gap_cnt - 3'd1;
        else if (Sdr_rd_en_aligned)
            frame_gap_cnt <= 3'd0;
    end
end

assign Sdr_rd_en_for_udp = (frame_gap_cnt == 3'd0) ? Sdr_rd_en_aligned : 1'b0;
...
//     .rst         (reset),
//     .pixel_valid (Sdr_rd_en_aligned),
//     .frame_start (bbox_frame_start),
//     .frame_end   (bbox_frame_end),
//     .has_bbox    (bbox_has),
//     .bbox_min_x  (bbox_min_x_prev_w),
//     .bbox_max_x  (bbox_max_x_prev_w),
//     .bbox_min_y  (bbox_min_y_prev_w),
//     .bbox_max_y  (bbox_max_y_prev_w),
//     .divider_x0  (divider_x0_w),
//     .divider_x1  (divider_x1_w),
//     .divider_x2  (divider_x2_w),
//     .divider_x3  (divider_x3_w),
//     .divider_x4  (divider_x4_w),
//     .divider_x5  (divider_x5_w),
//     .is_blue     (image_is_klein_blue),
//     .digit0      (digit_value0),
//     .digit1      (digit_value1),
//     .digit2      (digit_value2),
...
assign seg_data = 7'b1111111;
assign seg_sel  = 5'b11111;

wire [23:0] image_data;
assign image_data = {image_b_overlay, image_r_overlay, image_g_overlay};
//============================================================
// 时序对齐结束
//============================================================
//------------------------------------------------------------
...
udp_loopback#(
 ...
 (
    ...
    .app_rx_data_valid          (Sdr_rd_en_for_udp ),  // 使用对齐后的信号
