...
wire [3:0] saturation_digit;

image_saturation_analyzer #(
    .FRAME_WIDTH  (H_CMOS_DISP),
    .FRAME_HEIGHT (V_CMOS_DISP)
) u_image_saturation_analyzer (
    .clk          (sdr_clk),
    .rst_n        (rst_n),
    .pixel_valid  (Sdr_rd_en_aligned),
    .pixel_data   (image_data),
    .result_digit (saturation_digit)
);

seven_seg_static_display u_seven_seg_static_display (
    .digit   (saturation_digit),
    .seg_sel (slect_aw),
    .seg_data(a_data_outw)
);
//============================================================
// 时序对齐结束
//============================================================
