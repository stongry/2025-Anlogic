...
reg [18:0]w_addr_cnt;
	always @(posedge clk or negedge rst_n)
		begin
			...
			// 修复：简化逻辑，当wr_done为1且新数据到来时才清零
			else if(wr_done & !App_wr_en) begin
			// 写入完成后等待，保持wr_done=1
			w_addr_cnt <= 19'b0;  // 保持地址为0准备下一帧
			wr_done <= 1'b1;
...
reg [19:0]sdr_rd_delay;  // 增加到20位以支持更长延迟
	always @(posedge clk or negedge rst_n)
		begin
			if(!rst_n) begin
			sdr_rd_delay <= 20'b0;
			end
			// 当wr_done清零时，复位延迟计数器准备下一帧
			else if(!wr_done) begin
			sdr_rd_delay <= 20'b0;
			end
			// 激进优化：延迟时间2500周期（20μs@125MHz），进一步提升读取效率
			else if(wr_done & sdr_rd_delay < 20'd3500)
...
reg rd_done;//synthesis keep
	always @(posedge clk or negedge rst_n)
		begin
			...
			else if(!Sdr_busy & r_addr_cnt < 19'd307199 &  (sdr_rd_delay >= 20'd3500) & (udp_wrusedw < 'd3980) & Sdr_init_ref_vld == 1'b0 & burst_sdr_rd_cnt == 2'b0 )
			begin
            r_addr_cnt <=r_addr_cnt +1'b1;
			app_rd_en_reg <= 1'b1;
            end
			else if (!Sdr_busy & r_addr_cnt < 19'd307199 &  (sdr_rd_delay >= 20'd3500) & (udp_wrusedw < 'd3980) & Sdr_init_ref_vld == 1'b0 & (burst_sdr_rd_cnt > 0 &  burst_sdr_rd_cnt < 3))
			begin
            r_addr_cnt <=r_addr_cnt +1'b1;
			app_rd_en_reg <= 1'b1;
            end
			else if (!Sdr_busy & r_addr_cnt < 19'd307199 &  (sdr_rd_delay >= 20'd3500) & (udp_wrusedw < 'd3980) & Sdr_init_ref_vld == 1'b0 & burst_sdr_rd_cnt == 3)
			begin
			app_rd_en_reg <= 1'b0;
            end
            else if(!Sdr_busy & r_addr_cnt == 19'd307199 & (sdr_rd_delay >= 20'd3500) & (udp_wrusedw < 'd3980)& Sdr_init_ref_vld == 1'b0) 
			begin
			r_addr_cnt <=r_addr_cnt+1'b1;
			rd_done <=1'b1;
            app_rd_en_reg <= 1'b1;
