...
// RAM_WRITE,地址和使能。当收到ready信号时，把卷积核的结果数据写     
    output reg[10:0]ram_waddr,       //	.addra(camera_addr[15:0]), //写的地址//这里也是写地址的计数器，数到49*39-1=413
    output reg ram_wen,              //	.cea(camera_wrreq), //写请求，等到收到核的ready信号就拉高这个
// 卷积核的
    input          POOL_ready,                //收到CORE的ready的posedge信号后，控制cnt_ready+1当数到49*39-1=413时把ram_ren置0， 
    ...
    reg [11:0]cnt_ready;  //计数器，从49*39-1=413，每一个ready的posedge信号+1 //
 ...
 //======================这里使用的是2*2的===========
    ...
    always@(posedge  POOL_ready or negedge rstb_n)begin
      if(rstb_n == 1'b0)begin //复位
            ...
            if( cnt_ready !=  413 && ram_raddr != 0)              //49*39-1=413
                cnt_ready <=  cnt_ready + 1'b1 ;
            else if( cnt_ready == 413 )   //49*39-1=413
                cnt_ready <= 0 ;
      end
    end
    ...
    always@( posedge clk_b or negedge rstb_n )begin  //读部分
     if(rstb_n == 1'b0  || ram_ren == 1'b0 )begin //复位
       ...
       if(ram_ren==1'd1)begin
        ram_raddr <= cnt_l+cnt_h*47+cnt+cnt_n*45;//计算地址     //  98*78-1=7643 
    //地址相关计数器跳转 
          begin
             if(cnt>= 0 && cnt<=2) 
                  cnt <= cnt+1; 
          ...
          if(cnt == 3)begin
             if(cnt_l >= 0 && cnt_l <= 42)        // 98*78-1=7643 
                  cnt_l <= cnt_l + 2 ; 
             else if( cnt_l == 44 )                      // 98*78-1=7643
                  cnt_l <= 0;
          end
          if(  cnt==3 && cnt_l == 44 )begin         // 98*78-1=7643 
             if(cnt_h >= 0 && cnt_h <= 32)     //198*162-1=32075,7D4B//98*78-1=7643 
                  cnt_h <= cnt_h + 2  ;
             else if( cnt_h == 34  )                   //198*162-1=32075,7D4B //98*78-1=7643  
                  cnt_h <= 0;
          end
       end
     end      
   ...
   //=============================        
    always@( posedge clk_b or negedge rstb_n )begin//写入数据 ram_waddr和写使能ram_wen
      if(rstb_n == 1'b0)begin //复位
            ...
            if( POOL_ready == 1 && ram_waddr != 412  && ram_raddr != 0 )begin         ///49*39-1=413
               ram_waddr <=cnt_ready - 1'b1 ;
               ram_wen<=1; 
            end
             else if( ram_waddr == 412 &&  POOL_ready ==1 )begin         ///49*39-1=413
               ram_waddr<=413 ;                                                                    // /49*39-1=413
               ram_wen<=1; 
            end
            else if( ram_waddr == 413 )begin           //1/49*39-1=413
               ram_waddr<=0;
               ram_wen<=0; 
            end
      end
