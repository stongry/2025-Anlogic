...
//------------------------------------------------------------------- 
//*********************************************************************/
//  `define UDP_LOOP_BACK  // 注释掉，启用发送模式（TPG模式）
  `define DEBUG_UDP
...
parameter  LOCAL_UDP_PORT_NUM = 16'h0002;       // 本地端口2（发送端）
parameter  LOCAL_IP_ADDRESS   = 32'hc0a8f002;   // 本地IP 192.168.240.2（发送端）
parameter  LOCAL_MAC_ADDRESS  = 48'h0123456789ab;
parameter  DST_UDP_PORT_NUM   = 16'h0001;       // 目标端口1（接收端）
parameter  DST_IP_ADDRESS     = 32'hc0a8f001;   // 目标IP 192.168.240.1（接收端）
...
// 按键控制相关信号
wire signed [8:0] digit_value;  // 修改为9位有符号数：-100到+100
wire         digit_changed;
wire         led_control_changed;  // LED控制变化信号
wire [3:0]   led_control_value;    // LED控制值
...
key_counter key_counter_inst(
    ...
    .led_control_value(led_control_value),
    .led_control_changed(led_control_changed)
);

// UDP按键数据生成器 - 发送按键计数值和LED控制到远程设备
udp_key_data_tpg u1_udp_key_data_tpg(
    .clk                (udp_clk            ),
    .reset              (~key1              ),
    .digit_value        (digit_value        ),  // 来自按键计数器的数字值
    .digit_changed      (digit_changed      ),  // 数字改变触发信号
    .led_control_value  (led_control_value  ),  // LED控制值
    .led_control_changed(led_control_changed),  // LED控制变化触发信号
    .tpg_data_enable    (1'b1               ),  // 使能信号 - 常开

    .tpg_data           (tpg_data           ),  // 数据输出
    .tpg_data_valid     (tpg_data_valid     ),  // 数据有效信号
    .tpg_data_udp_length(tpg_data_udp_length),  // 数据长度（包含帧头）
    .tpg_data_done      (tpg_data_done      )   // 发送完成信号
);
