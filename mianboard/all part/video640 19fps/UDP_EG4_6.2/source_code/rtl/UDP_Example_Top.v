...
reg        cmos_vsync_d0;
reg        cmos_vsync_d1;
reg        frame_head_pending;
reg        frame_tail_pending;
reg        sdr_wr_en_r;
reg [23:0] sdr_wr_data_r;

wire frame_head_pulse = ~cmos_vsync_d0 &  cmos_vsync_d1; // VSYNC falling -> new frame
wire frame_tail_pulse =  cmos_vsync_d0 & ~cmos_vsync_d1; // VSYNC rising  -> frame end
wire inserting_head   = Sdr_init_done && frame_head_pending;
wire inserting_tail   = Sdr_init_done && ~frame_head_pending && frame_tail_pending;
wire        full_flag_sdr;
wire        full_flag;
...
assign sdr_wr_en   = sdr_wr_en_r;
assign sdr_wr_data = sdr_wr_data_r; // include frame markers
...
always @(posedge cam_pclk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        cmos_vsync_d0 <= cmos_frame_vsync;
...
always @(posedge cam_pclk or negedge rst_n) begin
    if(!rst_n) begin
        frame_head_pending <= 1'b0;
        frame_tail_pending <= 1'b0;
    end
    else begin
        if(frame_head_pulse)
            frame_head_pending <= 1'b1;
        else if(inserting_head)
            frame_head_pending <= 1'b0;

        if(frame_tail_pulse)
            frame_tail_pending <= 1'b1;
        else if(inserting_tail)
            frame_tail_pending <= 1'b0;
    end
end
...
always @(posedge cam_pclk or negedge rst_n) begin
    ...
    else if(inserting_head) begin
        sdr_wr_en_r   <= 1'b1;
        sdr_wr_data_r <= 24'hAA55AA;
    end
    else if(inserting_tail) begin
        sdr_wr_en_r   <= 1'b1;
        sdr_wr_data_r <= 24'h55AA55;
    end
    else begin
        sdr_wr_en_r   <= Sdr_init_done && cam_write_en;
        sdr_wr_data_r <= cam_write_data[31:8]; // {R,G,B}
    end
end
...
assign image_data = {image_b, image_r, image_g};
//============================================================
// 时序对齐结束
//============================================================
//------------------------------------------------------------
...
udp_loopback#(
 ...
 (
    ...
    .app_rx_data_valid          (Sdr_rd_en_aligned ),  // 使用对齐后的信号
