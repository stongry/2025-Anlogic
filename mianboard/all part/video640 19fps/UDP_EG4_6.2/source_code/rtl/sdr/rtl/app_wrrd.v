...
reg empty_flag_reg;
	...
	always @(posedge clk or negedge rst_n)           
		begin                                        
			if(!rst_n)
				wr_fifo_rd_en <=1'b0;
			else if(!Sdr_busy  & (Sdr_init_ref_vld == 1'b0) & !frame_available & (w_addr_cnt <= 19'd307199) & (rdusedw>4)) // hold off until previous frame is sent
				wr_fifo_rd_en <= 1'b1;
			else if(!Sdr_busy  & (Sdr_init_ref_vld == 1'b0) & !frame_available & (w_addr_cnt == 19'd307196) & (rdusedw==4))
				wr_fifo_rd_en <= 1'b1;
...
reg rd_done;//synthesis keep
	always @(posedge clk or negedge rst_n)
		begin
			...
			else if(!Sdr_busy & r_addr_cnt < 19'd307199 &  (sdr_rd_delay== 1000) & (udp_wrusedw < 'd3500) & Sdr_init_ref_vld == 1'b0 & burst_sdr_rd_cnt == 2'b0 )                                
			begin
            r_addr_cnt <=r_addr_cnt +1'b1;										 
			app_rd_en_reg <= 1'b1;
            end
			else if (!Sdr_busy & r_addr_cnt < 19'd307199 &  (sdr_rd_delay== 1000) & (udp_wrusedw < 'd3500) & Sdr_init_ref_vld == 1'b0 & (burst_sdr_rd_cnt > 0 &  burst_sdr_rd_cnt < 3))
			begin
            r_addr_cnt <=r_addr_cnt +1'b1;										 
			app_rd_en_reg <= 1'b1;
            end
			else if (!Sdr_busy & r_addr_cnt < 19'd307199 &  (sdr_rd_delay== 1000) & (udp_wrusedw < 'd3500) & Sdr_init_ref_vld == 1'b0 & burst_sdr_rd_cnt == 3)
			begin									 
			app_rd_en_reg <= 1'b0;
            end
            else if(!Sdr_busy & r_addr_cnt == 19'd307199 & (sdr_rd_delay== 1000) & (udp_wrusedw < 'd3500)& Sdr_init_ref_vld == 1'b0) 
			begin
			r_addr_cnt <=r_addr_cnt+1'b1;
			rd_done <=1'b1;
            app_rd_en_reg <= 1'b1;
